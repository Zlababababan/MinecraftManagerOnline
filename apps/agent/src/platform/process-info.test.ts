...
// Capacité d'introspection de l'hôte : sur certains runners CI Windows, la requête CIM échoue ou
// expire — l'agent retombe alors sur « rien de vérifiable » (comportement voulu, `verified: false`)
// et ces tests n'ont plus rien à mesurer.
const selfInfo = await getProcessInfo(process.pid);
const canIntrospect = selfInfo?.startedAt !== undefined && selfInfo.cmdline !== undefined;

describe('informations processus (ré-adoption, doc 05 §4)', () => {
  it.runIf(canIntrospect)(
    'lit l’heure de démarrage et la ligne de commande du processus courant',
    async () => {
  ...
  it.runIf(canIntrospect)(
    'vérifie l’identité d’un enfant détaché : heure + clé de commande',
    async () => {
