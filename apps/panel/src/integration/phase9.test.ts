...
import { buildTarGz, buildZip, testBudget } from '../../../agent/src/test/helpers.js';
import {
  connectClient,
  createTestPanel,
  freePort,
...
describe('phase 9 — panel ↔ deux agents réels', () => {
  ...
  it(
    ...
    async () => {
      ...
      expect(vendorHits.some((h) => h.startsWith('/vendor/v3/assets/latest/17/hotspot'))).toBe(
        true,
      );
  ...
  it(
    'releases : publication signée, agent.update accepté, signature invalide refusée, updateResult',
    async () => {
