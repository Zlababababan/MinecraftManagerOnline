 ...
 *   mmo-panel report               fichier de diagnostic à joindre à une issue (secrets exclus)
...
if (command === 'report') {
  const { runReportCommand } = await import('./cli-report.js');
  process.exit(await runReportCommand(config, process.argv.slice(3)));
}
