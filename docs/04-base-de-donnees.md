...
> **Amendement (2026-09-04, lot 5) — `servers.provisioning` : `install_failed` est enfin écrit, et il est terminal.** La valeur existait dans le CHECK depuis la phase 2 sans que rien ne la pose. Elle est désormais écrite par le service d’installation quand la task `server.install` échoue, et **aucun scan ne la lève** : la détection ne promeut en `ready` qu’un serveur `installing`, et seulement si aucune installation n’est en cours pour lui (`installInProgress`). Sans cette seconde condition, un scan tombant sur un dossier à moitié rempli déclarait le serveur démarrable **pendant** son installation. Un serveur créé par le panel naît `installing` / `detected = 0` / `desiredState = stopped` (sinon `restoreOnBoot` tenterait de démarrer une installation en cours) et n’est confirmé qu’à l’issue de la task, à partir du `DetectedServer` que l’agent renvoie. Aucune migration : ni colonne ni table nouvelle.

> **Amendement (2026-09-04, recette du lot 5) — `servers.path` s'écrit de deux façons.** Le scanner de l'agent joint le répertoire surveillé et le nom du sous-dossier avec « / » **même sous Windows** (`joinPath` de `@mmo/shared` est pur, sans `path.sep`), tandis que le panel compose avec le séparateur de l'OS quand il crée un serveur (lot 5) ou un clone : `C:srvmc/survie` et `C:srvmcsurvie` désignent le même dossier. `ServersService.findByPath` compare donc d'abord à l'identique (l'index UNIQUE `(machine_id, path)` fait le travail) puis, à défaut, sur une forme normalisée (séparateurs et casse). Sans ce repli, le panel ne reconnaissait pas le serveur qu'il venait lui-même d'installer et signalait un **conflit de marqueur** sur trois serveurs sains — vu en recette réelle sur un poste Windows. La colonne garde la valeur telle que la source l'a écrite : la normalisation est une comparaison, pas une réécriture.
