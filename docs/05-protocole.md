...
| `backup.verified` | A→P (event) | Verdict d'une **relecture complète** d'archive par l'agent (passe périodique, ou contrôle préalable d'une restauration) : `ok` ou corrompue, tailles et empreintes mesurées/attendues — **non critique** (le manifeste porte le verdict, `backup.list` le rattrape) |
| `backup.browse` | P→A | **Lot 4.** Contenu d'une archive lu par l'agent **sans extraction** (une passe sur les en-têtes tar) : dossiers avec taille et nombre de fichiers agrégés, fichiers (2 000 par dossier au plus, 50 000 entrées en tout, `truncated`) |
| `backup.restorePaths` | P→A | **Lot 4.** Task. Restauration **partielle** de `paths[]` (un dossier vaut tout son contenu) : `side_by_side` (défaut — extraction dans `restored-<date>/` à la racine du serveur, rien n'est remplacé, serveur non arrêté) ou `in_place` (chemins supprimés puis réécrits, arrêt, sauvegarde de sécurité, relance optionnelle) |
| `backup.receive` | P→A | **Lot 4.** Task. La machine reçoit une **copie hors-site** d'une archive produite ailleurs : sources (listener direct de l'agent source via `transfer.serve`, puis relais panel) avec reprise `Range`, sha256 vérifié, manifeste réécrit à côté sous le même `backupId`, rotation `keep` du dossier de copie ; résultat `{ archivePath, sizeBytes, sha256, source, rotated[] }`. Même message pour **rapatrier** une copie vers la machine du serveur |
...
> **Amendement (2026-09-02, lot 4) — les archives sont relues, pas seulement écrites.** Une sauvegarde n'était prouvée qu'à la restauration, c'est-à-dire trop tard. L'agent porte désormais un **`BackupVerifier`** (`apps/agent/src/backup/verifier.ts`) : une passe par jour (jamais avant 10 min après le démarrage), archives **jamais vérifiées d'abord, les plus anciennes en tête**, puis celles vérifiées il y a plus de 7 j ; **budget de 8 Gio relus par passe** (au moins une archive — un parc de 56 serveurs × 7 archives est couvert en semaines, pas en années, sans monopoliser le disque une nuit entière) ; **jamais pendant une task du même serveur** (`backup.*`, `migration.*`) ; une archive déclarée corrompue n'est plus relue (elle le reste jusqu'à sa suppression). Une restauration compte aussi comme une vérification (elle relit tout). **Extension par ajout, sans bump** : `backupManifestSchema` gagne `verifiedAt?` et `verifyStatus? ∈ ok | corrupted`, **écrits dans le manifeste** par l'agent ; nouvel événement `backup.verified { serverId, backupId, ts, ok, sizeBytes, sha256, expectedSizeBytes, expectedSha256, archivePath? }`, **non critique** pour la même raison que `backup.skipped` — et c'est le manifeste qui assure la durabilité : `backup.list` à chaque reconnexion rapporte le verdict, un événement perdu est rattrapé là (prouvé : verdict effacé en base, agent reconnecté, verdict rétabli). Côté panel : colonnes `backups.verified_at`/`verify_status` (migration `0013`), `recordVerification`, réconciliation étendue au verdict, événement de bus **`backup.corrupted`** (`severity: error`, catégorie de notification `backup.failed`) publié **une seule fois par archive** — à la transition, quelle que soit la voie par laquelle le panel l'apprend. Un agent N-1 ne vérifie rien : ses manifestes n'ont pas de verdict, l'UI affiche « pas encore vérifiée ». Pas de requête `backup.verify` à la demande : la passe suffit, et un bouton par archive serait un réflexe de plus à avoir.

> **Amendement (2026-09-02, lot 4) — restaurer un monde seul, sans tout écraser.** Une restauration était tout ou rien : pour récupérer une région corrompue ou le `server.properties` d'hier, il fallait remplacer le dossier entier. Deux requêtes **ajoutées sans bump** (`packages/protocol/src/messages/tasks.ts`) : **`backup.browse { serverId, backupId, archivePath? }`** → `{ entries[], totalFiles, totalBytes, truncated }` — l'agent lit les **en-têtes tar** de l'archive décompressée en flux sans écrire un octet (`listTar` de `@mmo/shared/node`, même détection d'archive tronquée que l'extraction) ; chaque dossier porte la taille et le nombre de fichiers qu'il contient (récursif), au plus `BROWSE_FILES_PER_DIR` (2 000) fichiers sont listés par dossier et `BROWSE_MAX_ENTRIES` (50 000) entrées en tout, les dossiers passant toujours en premier — un dossier coché se restaure entier quoi qu'il soit listé dessous. Pas de cache côté agent (une passe par appel, le navigateur garde la réponse 5 min). **`backup.restorePaths { taskId, serverId, backupId, archivePath?, paths[] (1–500, relatifs, jailés), mode = side_by_side | in_place, safetyBackup = true, safetyBackupId?, restartAfter = false }`** (task, `TASK_KINDS`) : l'inclusion est un **prédicat par préfixe** posé sur `extractTar` (`ExtractOptions.include`, appliqué au chemin relatif jailé — un chemin choisi vaut lui-même et tout ce qu'il contient). Gardes, toutes avant le premier geste irréversible : chemins normalisés et **jamais réservés** (marqueur, journaux, corbeille, `session.lock`, dossiers `restored-*` — refus `E_INVALID_PAYLOAD` `details.reason = RESERVED_PATH` **à la requête**, 400 côté panel, pas une task en échec) → archive conforme à son manifeste (`E_CHECKSUM_MISMATCH`, comptée comme vérification) → **chaque chemin demandé existe dans l'archive** (`E_NOT_FOUND` `PATHS_NOT_IN_ARCHIVE` : en place, un chemin absent aurait supprimé le dossier courant pour ne rien remettre) → espace libre (côte à côte, `E_IO` `INSUFFICIENT_SPACE`). **`side_by_side` est le défaut** : extraction dans `restored-<yyyymmdd>-<hhmmss>[-n]/` à la racine du serveur, sans arrêt, sans sauvegarde de sécurité, rien de remplacé — la discipline actée sur des serveurs réels veut que le mode par défaut n'écrase jamais ; ce dossier est ignoré du scanner (`RESTORED_DIR`, comme `.migrated-*`), exclu des archives et épargné par une restauration complète (comme les journaux) : il appartient à l'utilisateur, qui déplace ce dont il a besoin ou le supprime. `in_place` : arrêt si en marche (watchdog annulé), sauvegarde de sécurité `pre_restore` (« before partial restore of <id> »), journal WAL, `rm` de chaque chemin choisi, extraction filtrée, relance optionnelle. Résultat `{ backupId, mode, paths (normalisés, dédoublonnés — `world` couvre `world/region`), destination? (côte à côte), files, bytes, safetyBackup?, restarted, wasRunning }`. Exclusivité `E_BUSY` avec les autres tasks de sauvegarde du serveur (agent, planning, vérificateur, panel). Capacité `partial-restore` ; un agent N-1 répond `E_UNSUPPORTED_TYPE` aux deux requêtes — le panel rend 501, l'UI dit de mettre l'agent à jour. Panel : `GET /api/servers/:id/backups/:backupId/browse` (lecture, relais tel quel), `POST …/restore-paths` (opérateur, task `backup.restorePaths`, ligne `pre_restore` seulement en place avec sécurité, audit `backup.restorePaths`). Web : « Restaurer des fichiers… » dans le menu d'une archive → arbre à cases (composant `Tree` de Mantine, sélection par préfixe : un dossier coché grise ses enfants, un enfant coché rend le parent indéterminé), filtre par nom, mode côte à côte présélectionné.

> **Amendement (2026-09-02, lot 4) — copie hors-site vers une autre machine du parc.** Une requête **ajoutée sans bump**, `backup.receive { taskId, serverId, backupId, manifest, sources[] (url, kind direct|relay, headers?), destination?, keep?, connectTimeoutMs? }` → `{ taskId }` (capacité `replication` ; un agent N-1 répond `E_UNSUPPORTED_TYPE`, relayé en 501), résultat de task `{ backupId, serverId, archivePath, sizeBytes, sha256, source, durationMs, rotated[] }`. **Agent** (`BackupService.receive`) : même garde de marqueur qu'une sauvegarde locale sur une destination explicite, garde d'espace (taille connue + marge), `downloadWithResume` (sha256 vérifié, `.part` supprimé au désaccord), nom de fichier **recalculé** `<backupId>.<ext>` (le chemin d'origine peut venir d'un autre OS), manifeste réécrit `{ …manifest, archivePath }` sous le même `backupId` — `backup.list`, `transfer.serve` et `backup.delete` savent donc la servir sans que le serveur existe sur cette machine —, puis rotation `keep` du dossier de copie (rang par `createdAt`, jamais la copie qu'on vient d'écrire). Une copie déjà présente (rejeu du même `taskId`) est revérifiée, pas retéléchargée. **Panel** (`services/replication.ts`, tables `backup_replication` / `backup_replicas`, doc 04 §5) : réglage par serveur (machine ≠ celle du serveur, `keepLast`, interrupteur), copie ordonnée après chaque `applyManifest` d'une archive `manual`/`scheduled` (jamais `pre_migration`/`pre_restore`), jamais deux fois la même archive vers la même machine ; sources = `transfer.serve` sur la machine qui détient l'archive + jeton relais (kind `migration`, `migrationId` = id de la task, révoqué à l'issue) ; issue par `onTaskFinished` (copie `success`/`failed`, `rotated[]` → copies `deleted`) ; **rapatriement** = même message dans l'autre sens (la destination sert, la machine du serveur reçoit, `applyManifest` remet la fiche en `success`) ; suppression de l'original → copies joignables supprimées (`backup.delete`), les autres restent et gardent la fiche visible ; réconciliation à la reconnexion de la destination (`backup.list` par serveur concerné : copies disparues → `deleted`, copies retrouvées → réinsérées) puis **rattrapage** (la dernière archive saine sans copie part). Libellés de notification propres (`replicaDone`/`replicaFailed`, catégories `task.done`/`task.failed` inchangées). Routes : `PUT /api/servers/:id/replication`, `POST …/backups/:backupId/replicate`, `POST …/replicas/:replicaId/pull`, `DELETE …/replicas/:replicaId` ; `GET …/backups` gagne `replication` et `replicas` et liste aussi une fiche `deleted` tant qu'une copie saine existe. Tests : `http/replication.test.ts` (faux agents), `integration/phase9.test.ts` (deux agents réels : copie en direct, rotation sur la destination, rapatriement puis restauration), `agent.backup.test.ts` (source HTTP, copie présente revérifiée, archive altérée, `keep`, marqueur).
