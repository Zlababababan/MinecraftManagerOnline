...
>
> **Phase 12 — purges, sauvegarde du panel, accessibilité (2026-08-23)** : **restauration** du panel livrée (`mmo-panel restore <fichier>` → `restorePanelBackup()`, doc 04 §8.4, guide §1.7) + carte « Sauvegardes du panel » dans Réglages (les hooks existaient sans UI) ; test `integration/panel-backup.test.ts` (VACUUM INTO → dérive → restauration → agent reconnecté avec son secret, serveur ré-adopté avec le même ID, démarrage/arrêt OK ; purges sur horloge simulée : sessions, codes, événements 90 j, audit 365 j, rotation 7 copies, `backupIfStale` 24 h, `retention.eventsDays` honoré). **Accessibilité** : `e2e/a11y.spec.ts` (axe-core, WCAG 2.1 A/AA, 7 pages × clair/sombre × 4 projets) — zéro violation `serious`/`critical` après correctifs : contrastes (thème `primaryShade 9` + `autoContrast`, teal-9/green-9 assombris, `dimmed`/placeholder relevés, nav basse sombre, `.mmo-warn-text`), `Popover.Target` sur le bouton et non sur l'`Indicator` (aria-* interdits sur un div), `aria-label` des `Progress` et des SVG des graphiques, nom des `CloseButton` (notifications/modales) via `CloseButton.extend`. E2E : **42/42**.
>
> **Phase 12 — release 1.0.0 (2026-08-23)** : versions agent/panel **1.0.0**, clé de release générée (doc 03 §3 : privée hors dépôt, publique dans `RELEASE_KEYS` ; les builds `--release` retirent la clé de dev via `MMO_RELEASE_BUILD`), `release/1.0.0/` reconstruit avec `--release --panel` (bundle signé clé mainteneur, 4 plateformes + panel win-x64) et `smoke.mjs` vert (`launcher --version` = agent 1.0.0). `pnpm check` : 20 tâches, 411 tests (seul `sampler.test.ts` flake sous charge, vert isolément), e2e **42/42**. Tag `v1.0.0` posé. **Restent à la main après le tag** (doc 07 « Restent à jouer », inchangé) : reboot Windows (installation laissée en place), relais `sudo` interactif d'`install.sh`, Linux ARM réel, launchd macOS, migration Windows → Linux ARM, push Android/iOS réels, accès hors réseau ; dette sécurité résiduelle actée doc 03 §6.
>
...
2. **Un spike précède le lot 5** sur `runJar` seul (Forge 1.12, Forge 1.20, NeoForge, sur des copies) : c'est le seul inconnu réel — un programme Java tiers qui télécharge de lui-même, dure des minutes et exige strictement Java 8 jusqu'en 1.16.5. → **Fait le 2026-09-04, mesures en doc 06 §6bis, et la prémisse était fausse** : l'installeur n'est pas soumis à la contrainte Java du serveur (Forge 1.20.1 et NeoForge 21.1 s'installent sous Java 8, Forge 1.12.2 sous Java 21). L'agent installe donc avec le Java qu'il a, et ne résout le bon qu'au premier démarrage — un prérequis en moins. Retenu aussi : code de retour fiable (0/1) doublé d'une ligne finale explicite, jusqu'à 7 580 lignes de sortie à NE PAS relayer en console, progression par phases et non en pourcentage, et rien d'autre que les fichiers des templates du §1 (ni `eula.txt`, ni `server.properties`).
**Première moitié du lot 5 livrée le 2026-09-04** (vanilla et Fabric) : catalogues purs dans `@mmo/shared`, message `server.install` (ajout sans bump, capacité `server-install`), exécuteur côté agent, création et reprise côté panel, assistant en quatre écrans. Le squelette y est en entier — planificateur, `install_failed` terminal, mode « réparer » — et la seconde moitié (Forge, NeoForge, modpacks, CurseForge, Paper) n'a plus qu'à ajouter des étapes au plan. Mesures du lanceur Fabric en doc 06 §6ter ; le huitième chantier du lot 8 (« qui a le droit de créer un serveur ») est tranché avec, en doc 02 §9.
