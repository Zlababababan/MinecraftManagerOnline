...
Périmètre : **tout ce qui est dans le dépôt aujourd'hui**, c'est-à-dire la 1.0.7 publiée plus les chantiers non publiés des lots 9, 4, 8, la première moitié du lot 5 et les correctifs des 12–14 septembre issus de la première passe de cette fiche (section « Unreleased » du CHANGELOG). Chaque étape = **une action → le résultat attendu**. Cochez au fur et à mesure ; notez tout écart, même cosmétique, avec le numéro de l'étape et l'heure (pour croiser avec les journaux). Les étapes marquées **(corrigé le …)** ont échoué à la première passe et sont à rejouer.
...
| Quoi                        | Où                                                                                                                                                                                                                  |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Panel (code)                | `E:\mmo-panel\mmo-panel-1.0.8\` (archive construite depuis `main` le 14 septembre — version affichée **1.0.7** : rien n'est tagué)                                                                                  |
| Panel (données)             | `E:\mmo-panel\mmo-panel-1.0.8\data\` — `mmo.db`, `metrics.db`, `logs\`, `backups\panel\`, `releases\`                                                                                                               |
| Journal du panel            | `E:\mmo-panel\mmo-panel-1.0.8\data\logs\panel-<date>.log` (NDJSON — lisible en console si lancé depuis un terminal)                                                                                                 |
| Icône près de l'horloge     | `E:\mmo-panel\mmo-panel-1.0.8\app\install\mmo-panel-tray.ps1`                                                                                                                                                       |
...
| Anciennes versions          | `E:\mmo-panel\mmo-panel-1.0.7\` (première passe, intact, données du 12 septembre), `E:\mmo-panel\mmo-panel.old\`, `E:\mmo-panel\mmo-panel-1.0.5-win-x64\`, `E:\mmo-panel\mmo-panel-sauvegarde-avant-demenagement-…` |
...
- [ ] **0.2** Panel arrêté, ouvrir un terminal (PowerShell ou cmd) dans `E:\mmo-panel\mmo-panel-1.0.8\` et lancer `mmo-panel.cmd doctor`. Attendu : une ligne par contrôle (runtime, `node:sqlite`, argon2, écriture réelle dans `data\`, `quick_check` de la base, port 3000 libre, front), le niveau **en couleur** (`ok` vert, `warn` jaune, `ERROR` rouge — corrigé le 14 septembre, dans cmd aussi) et la conclusion « no blocking problem found » en vert.
- [ ] **0.3** Lancer le panel **depuis ce même terminal** : `start-panel.cmd`. Attendu : des lignes lisibles du type `22:49:09 INFO  panel ready users=1 dataDir=E:\mmo-panel\mmo-panel-1.0.8\data` (plus de JSON brut avec `pid`/`hostname`) ; **aucune ligne `request`** dans la fenêtre pendant que vous naviguez (corrigé le 12 septembre : le journal d'accès ne va plus qu'au fichier) ; le fichier `data\logs\panel-<date>.log` reçoit, lui, du NDJSON, lignes `request` comprises.
...
- [ ] **0.5** Fermer le terminal (Ctrl+C), puis relancer par l'icône : `powershell -WindowStyle Hidden -ExecutionPolicy Bypass -File "E:\mmo-panel\mmo-panel-1.0.8\app\install\mmo-panel-tray.ps1"`. Attendu : aucune fenêtre, icône près de l'horloge ; clic gauche → le navigateur s'ouvre sur le panel ; clic droit → Ouvrir / Journaux / Démarrer / Arrêter / Redémarrer / Démarrer avec Windows / Quitter ; « Journaux » ouvre `data\logs\`.
...
- [ ] **0.8** Le dossier `E:\mmo-panel\mmo-panel-1.0.8\` a été posé le 14 septembre : code construit depuis `main` (correctifs des 12–14 septembre compris) et **copie** des données de `mmo-panel-1.0.7\data\` prise panel arrêté ; le dossier 1.0.7 est intact. C'est **ce dossier** que toute la fiche utilise désormais (étapes 0.1 à 0.5, `report`, `restore`). Attendu : `/api/health` répond `"version":"1.0.7"` (rien n'est tagué), les 54 serveurs et l'agent sont là, l'agent se reconnecte seul (même URL, même appairage).

## 1. Agent 1.0.7

Contexte : à la première passe, l'agent du PC (build release 1.0.5) a été réinstallé **à la main** en 1.0.7 (`install.ps1 -Panel http://127.0.0.1:3000/`, étape 1.4 du 5 septembre) parce que le one-liner échouait. Il est donc déjà en 1.0.7, signé avec la **clé de développement** comme le bundle embarqué par ce panel. Il reste à rejouer le one-liner corrigé (1.3) — il réinstalle la même version et garde l'appairage — puis à vérifier ce que la 1.0.7 apporte côté agent (fichier de diagnostic, coût du processus agent, journal fichier).

- [ ] **1.1** Machines → le PC → carte **Agent**. Attendu : « Version de l'agent 1.0.7 », « Dernière release 1.0.7 », **aucune** mention de mise à jour disponible.
- [ ] **1.2** _(optionnel)_ Le bouton « Mettre à jour l'agent », s'il est proposé malgré des versions égales, doit refuser proprement (« déjà à jour » ou signature) sans faire tomber l'agent. S'il n'est pas proposé : passer.
- [ ] **1.3** _(corrigé le 12 septembre : le script servi commençait par un BOM que PowerShell refuse dans un `scriptblock`, en 5.1 comme en 7)_ Réglages → Distribution de l'agent → copier la commande **Windows** (sans code d'appairage) → la coller dans un **PowerShell administrateur**. Attendu : aucune erreur de parse ; « arrêt du service mmo-agent », téléchargement de `mmo-agent-1.0.7-win-x64.zip` depuis le panel, « service mmo-agent installé et démarré », **sans** redemander de code d'appairage.
...
- [ ] **3.6** _(corrigé le 12 septembre : un pack FTB fraîchement installé n'était vu qu'après un premier `run.bat`)_ Créer `E:\Minecraft\Server\test-argfiles\` en y copiant, depuis `FTBEvolution_1.43.1\`, **seulement** `run.bat`, `user_jvm_args.txt` et `libraries\net\neoforged\neoforge\21.1.248\win_args.txt` (en recréant les dossiers) — ni `eula.txt`, ni `server.properties`, ni `mods\`. Répertoires surveillés → « Scanner maintenant ». Attendu : « 55 serveur(s) détecté(s) », `test-argfiles` dans la liste avec loader **NeoForge 21.1.248**, Minecraft 1.21.1, mémoire 8092 Mio, et la mention **EULA non acceptée** ; Démarrer refuse avec la carte « EULA Minecraft » (ne pas accepter : le dossier n'a pas les bibliothèques). Puis « Oublier ce serveur » et supprimer le dossier.
- [ ] **3.7** _(corrigé le 12 septembre : « Conflit : la ressource a été modifiée entre-temps » ne disait rien)_ Répertoires surveillés → « Ajouter un dossier serveur » avec `E:\mmo-panel\mmo-panel-1.0.8\app`. Attendu : refus qui commence par « Aucun serveur Minecraft reconnu dans … » et dit ce qu'il faudrait au dossier (server.properties, eula.txt, jar + mods/, ou installation Forge/NeoForge) ; rien n'est ajouté à la liste.
...
- [ ] **4.9** _(corrigé le 12 septembre)_ Refus, en deux temps. (a) Lancer l'assistant avec le nom `test-vanilla` (déjà enregistré). Attendu : **dès l'écran Emplacement**, sous le champ, « Ce nom de dossier est déjà celui d'un serveur enregistré ici », et « Suivant » n'avance pas ; changer le répertoire ou le nom efface le message. (b) Créer à la main `E:\Minecraft\Server\test-occupe\note.txt`, puis l'assistant avec le nom `test-occupe`. Attendu : l'écran Vérification signale que le dossier existe et n'est pas vide ; en créant quand même, erreur immédiate **« Le dossier … existe déjà et n'est pas vide. Choisissez un autre nom de dossier. »** (plus de « Conflit : la ressource a été modifiée entre-temps »), **aucun serveur fantôme** dans la liste, `note.txt` intact.
...
- [ ] **6.4** _(corrigé le 12 septembre : les champs n'acceptaient que deux caractères)_ Macros → « Nouvelle macro » : taper le nom `Soir` **au clavier, lettre par lettre**, puis les commandes `time set night` et `say bonne nuit` (une par ligne), « Sur ce serveur seulement ». Attendu : la frappe entière est acceptée dans les deux champs, aucune erreur « Cannot read properties of null » ; la macro s'exécute dans l'ordre, message « « Soir » exécutée (2 commande(s)) ».
...
- [ ] **11.16** _(corrigé le 14 septembre : 57 avertissements « Aucune sauvegarde de … » à chaque redémarrage du panel)_ Tous les serveurs de test arrêtés, redémarrer le panel (icône → Redémarrer) et attendre 5 min. Attendu : **aucune** notification « Aucune sauvegarde de … » — une politique « seulement si le serveur tourne » d'un serveur arrêté n'est jamais en retard. Pour voir un vrai retard : `test-vanilla` **en marche**, sa politique par défaut modifiée à une heure déjà passée de plus de 2 h, redémarrer le panel → **une** notification pour ce serveur ; s'il y en avait plusieurs, une seule notification « N sauvegardes en retard » (chaque serveur garde son événement dans son onglet Événements).
...
- [ ] **15.7** Terminal dans `E:\mmo-panel\mmo-panel-1.0.8\` : `mmo-panel.cmd report`. Attendu : `data\mmo-report-<date>.txt` ; à l'intérieur ni votre nom d'utilisateur (`C:\Users\<user>`), ni chemin de serveur, ni valeur de jeton ; les adresses IPv4 sans dernier octet, `127.0.0.1` intact ; les réglages secrets « (set, hidden) ».
...
- Panel : `E:\mmo-panel\mmo-panel-1.0.8\data\logs\panel-<date>.log` (chaque requête y est tracée avec son `requestId`, le même que dans un message d'erreur 500) ; si lancé depuis un terminal, la sortie lisible — sans les lignes `request`, sauf avec `set MMO_LOG_CONSOLE=full` avant `start-panel.cmd`.
