...
- [ ] **0.8** Le panel de production doit porter les correctifs des 12–14 septembre (`main` après le commit « Console du panel : le journal d'accès reste dans le fichier »). Si ce n'est pas encore fait : demandez-le en début de session — l'archive est construite depuis `main` et posée à la place du code, les données ne bougent pas ; vous relancez ensuite par l'icône. Attendu après relance : `/api/health` répond toujours `"version":"1.0.7"`, les 54 serveurs sont là, et l'étape 0.3 ne montre plus de lignes `request`.
...
- Panel : `E:\mmo-panel\mmo-panel-1.0.7\data\logs\panel-<date>.log` (chaque requête y est tracée avec son `requestId`, le même que dans un message d'erreur 500) ; si lancé depuis un terminal, la sortie lisible — sans les lignes `request`, sauf avec `set MMO_LOG_CONSOLE=full` avant `start-panel.cmd`.
