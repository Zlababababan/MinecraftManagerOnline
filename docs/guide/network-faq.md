...
**English** · [Français](fr/faq-reseau.md) · [Español](es/network-faq.md) · [Deutsch](de/network-faq.md) · [Português](pt/network-faq.md) · [Русский](ru/network-faq.md) · [中文](zh/network-faq.md)
...
Troubleshooting: `tailscale status` on the host; `tailscale serve status` must list the proxy (`No serve config` = the serve command was never run — the reachability test then fails with a refused connection on port 443); if the WebSocket test fails while HTTP passes, check that you do not have another proxy in front (nginx) without `Upgrade`. If the terminal answers that `tailscale` is not recognized (Windows), the CLI is not in your PATH: call it with its full path in **double** quotes (single quotes break on Windows) — Command Prompt: `"C:\Program Files\Tailscale\tailscale.exe" serve …`, PowerShell: the same with `&` in front (adjust to your install folder).
