    ...
    { name: 'mmo-agent/install.sh', data: installSh, mode: 0o755 },
    {
      ...
      data: Buffer.from(
        [
          ...
          '',
          'Installation guidée : depuis le panel, « Ajouter une machine » donne la commande à coller.',
          'Installation hors ligne : install.ps1 (Windows) / install.sh (Linux, macOS) avec --archive <ce fichier>.',
          'Désinstallation : install.ps1 -Uninstall / install.sh --uninstall.',
          'Lancement manuel : runtime/<version>/node(.exe) launcher.cjs run --panel wss://<panel>/ws/agent',
          '',
        ].join('\n'),
      ),
    },
  ...
  entries.push({
    ...
    data: Buffer.from(
      [
        ...
        '',
        'Démarrage : mmo-panel.cmd (Windows) / ./mmo-panel.sh (Linux, macOS), puis http://127.0.0.1:3000',
        'Données : ./data (MMO_DATA_DIR) — sauvegardez ce dossier. Archives agent servies : ./dist-agent (MMO_DIST_DIR).',
        'Service et accès distant : docs/guide/installation.md',
        '',
      ].join('\n'),
    ),
  });
